...
dtype* execute_kernel (int n, int m, dtype* A, dtype* B, void (*kernel)(int, int, const dtype*, const dtype*, dtype*), int blk_ratio, float* Bandwidth, float* CompTime, double* Flops) {
    ...
    checkCudaErrors( cudaMemcpyAsync(dev_A, A, n*m*sizeof(dtype), cudaMemcpyHostToDevice, stream) );
    checkCudaErrors( cudaMemcpyAsync(dev_B, B, n*m*sizeof(dtype), cudaMemcpyHostToDevice, stream) );
    ...
    *Flops  = ( n * m ) / ( (*CompTime) * 1e+9 );
    ...
    dtype *GPU_C;
    checkCudaErrors(cudaHostAlloc(&GPU_C, sizeof(dtype) *n*m, cudaHostAllocDefault));
    TIMER_START;
    checkCudaErrors( cudaMemcpyAsync(GPU_C, dev_C, n*m*sizeof(dtype), cudaMemcpyDeviceToHost, stream) );
    TIMER_STOP;
...
//   printf("alpha = %f, beta = %f\n", alpha, beta);
  ...
  checkCudaErrors(cudaHostAlloc(&A, sizeof(dtype) *n*m, cudaHostAllocDefault));;
  checkCudaErrors(cudaHostAlloc(&B, sizeof(dtype) *n*m, cudaHostAllocDefault));;
  checkCudaErrors(cudaHostAlloc(&CPU_C, sizeof(dtype) *n*m, cudaHostAllocDefault));;
  checkCudaErrors(cudaHostAlloc(&GPU_C, sizeof(dtype) *n*m, cudaHostAllocDefault));;
  ...
  checkCudaErrors(cudaFreeHost(GPU_C));
